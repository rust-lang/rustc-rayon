...
impl WorkerThread {
    ...
    pub(super) unsafe fn wait_or_steal_until<L: AsCoreLatch + ?Sized>(
        ...
        if !latch.probe() {
            if steal {
                self.wait_or_steal_until_cold(latch);
            } else {
        ...
        'outer: while !latch.probe() {
            ...
            while !latch.probe() {
                if let Some(job) = self.find_work() {
                    ...
                    self.registry
                        .sleep
                        .no_work_found(&mut idle_state, latch, &self, true)
                }
            }
    ...
    unsafe fn wait_until_cold(&self, latch: &CoreLatch) {
        ...
        while !latch.probe() {
            self.registry
                .sleep
                .no_work_found(&mut idle_state, latch, &self, false)
        }
    ...
    unsafe fn wait_until_out_of_work(&self) {
        ...
        self.wait_or_steal_until(&registry.thread_infos[index].terminate, true);
